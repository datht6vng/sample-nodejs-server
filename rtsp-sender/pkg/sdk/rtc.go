...
func (r *RTC) onSingalHandle() error {
	for {
		...
		case *rtc.Reply_Join:
			...
			log.Debugf("payload.Reply.Description=%v", payload.Join.Description)
		...
		case *rtc.Reply_Description:
			...
			if sdp.Type == webrtc.SDPTypeOffer {
				log.Debugf("[%v] [description] got offer call s.OnNegotiate sdp=%+v", r.uid, sdp)
				err := r.negotiate(sdp)
				if err != nil {
					log.Errorf("error: %v", err)
				}
			} else if sdp.Type == webrtc.SDPTypeAnswer {
				log.Debugf("[%v] [description] got answer call sdp=%+v", r.uid, sdp)
...
func (r *RTC) SendOffer(sdp webrtc.SessionDescription) error {
	log.Debugf("[C=>S] [%v] sdp=%v", r.uid, sdp)
	go r.onSingalHandleOnce()
	r.Lock()
	err := r.signaller.Send(
		&rtc.Request{
