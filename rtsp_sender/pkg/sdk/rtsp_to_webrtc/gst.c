...
GstFlowReturn gstreamer_send_new_audio_sample_handler(GstElement *object, gpointer user_data) {
  ...
  if (sample) {
    ...
    if (buffer) {
      gst_buffer_extract_dup(buffer, 0, gst_buffer_get_size(buffer), &copy, &copy_size);
      goHandlePipelineAudioBuffer(copy, copy_size, GST_BUFFER_DURATION(buffer), s->pipelineId);
    }
    gst_sample_unref (sample);
  }
...
void gstreamer_send_start_pipeline(GstElement *pipeline, bool audioEnable, bool videoEnable, int pipelineId) {
  ...
  if (audioEnable) {
    ...
    g_signal_connect(appsink, "new-sample", G_CALLBACK(gstreamer_send_new_audio_sample_handler), s);
