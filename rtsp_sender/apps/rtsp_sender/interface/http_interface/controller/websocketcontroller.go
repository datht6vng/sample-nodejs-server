...
func (c *WebSocketController) Handle(con *websocket.Conn) {
	...
	if oldConnRaw, ok := c.conns.Load(con.RemoteAddr().String()); ok {
		if oldConn, ok := oldConnRaw.(*socketConnectionWrapper); ok {
			oldConn.Close()
		}
	}
	c.conns.Store(con.RemoteAddr().String(), conn)
	defer c.conns.Delete(con.RemoteAddr().String())
	...
	for {
		...
		case "join":
			...
			signaller, err := sfuClient.Signal(context.Background())
			if err != nil {
				onErr(req, err)
				break
			}
		...
		for {
			...
			default:
				...
				if err != nil {
					...
					logger.Errorf("[%v] Error receiving RTC response: %v", uid, err)
					return
				}
				resChannel <- reply
			}
